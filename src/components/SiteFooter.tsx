...
export function SiteFooter() {
  ...
  const [contactTopic, setContactTopic] = useState("Contact us");
  ...
  return (
    <footer className="mt-24 border-t border-border bg-card/60">
      <div className="max-w-6xl mx-auto px-6 py-12 grid gap-10 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li>
              <button onClick={() => openContact("Contact us")} className="hover:text-foreground text-left">
                Contact us
              </button>
            </li>
            <li><button onClick={() => openContact("Report a bug")} className="hover:text-foreground text-left">Report a bug</button></li>
            <li><button onClick={() => openContact("Partner with us")} className="hover:text-foreground text-left">Partner with us</button></li>
          </ul>
        </div>
      ...
      <ContactModal open={contactOpen} onClose={() => setContactOpen(false)} topic={contactTopic} />
    </footer>
  );
}
