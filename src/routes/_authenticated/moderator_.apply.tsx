    ...
    (async () => {
      ...
      if (uid) {
        ...
        const { data: prof } = await (supabase.from("moderator_profiles" as any) as any)
          .select("verified, ai_reason, organization, community")
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-card/95 backdrop-blur border-b border-border">
        ...
        <h1 className="font-display text-lg font-bold">Moderator onboarding</h1>
      </header>
        ...
        <section className="rounded-2xl border border-border bg-card p-5 space-y-3">
          ...
          {existing?.verified && (
            <div className="rounded-xl px-3 py-2 text-xs bg-success/10 text-success border border-success/30">
              You're already verified. You can still update your details below.
            </div>
          )}
        ...
        <form onSubmit={submit} className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div>
            <label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
              <Building2 size={12} /> Department or organization
            </label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
              <MailCheck size={12} /> Verified email
            </label>
          ...
          <div>
            <p className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mb-2">
              <ShieldCheck size={12} /> Employment proof (pick one)
            </p>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setProofKind("letter")}
                ...
                <div className="text-sm font-medium">Authorization letter</div>
                <div className="text-[11px] text-muted-foreground">Signed, with city-hall letterhead</div>
              </button>
              <button type="button" onClick={() => setProofKind("badge")}
                ...
                <div className="text-sm font-medium">City-hall ID badge</div>
                <div className="text-[11px] text-muted-foreground">Upload or snap your employee ID</div>
              </button>
            </div>
          ...
          <div className="rounded-3xl border border-border bg-card p-8 text-center max-w-sm">
            ...
            {state.phase === "approved" && (
              <>
                ...
                <h3 className="mt-4 font-display text-xl font-bold">You're verified!</h3>
                <p className="mt-2 text-sm text-muted-foreground">{state.reason}</p>
                <p className="mt-2 text-xs text-muted-foreground">Taking you to the moderator dashboard…</p>
              </>
            )}
            {state.phase === "rejected" && (
              <>
                <XCircle size={40} className="mx-auto text-destructive" />
                <h3 className="mt-4 font-display text-xl font-bold">We couldn't verify this document</h3>
                <p className="mt-2 text-sm text-muted-foreground">{state.reason}</p>
